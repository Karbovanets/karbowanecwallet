  ...
  bool Miner::worker_thread(uint32_t th_local_index)
  {
    ...
    while(!m_stop_mining)
    {
      ...
      if (!m_stop_mining && check_hash(pow, local_diff))
      {
        ...
        m_logger(Logging::INFO) << "Found block " << Common::podToHex(cb.getBlockHash()) << " at height " << cb.getBlockIndex() << " for difficulty: " << local_diff << ", POW " << Common::podToHex(pow);
        Q_EMIT minerMessageSignal(QString("%1 Found block %2 at height %3 for difficulty %4, POW %5").arg(formattedTime).arg(QString::fromStdString(Common::podToHex(cb.getBlockHash()))).arg(cb.getBlockIndex()).arg(local_diff).arg(QString::fromStdString(Common::podToHex(pow))));
